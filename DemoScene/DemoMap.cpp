...
namespace
{
	...
	// 背景オブジェクトの削除距離
	constexpr float kDeleteRange = 24000.0f;
	// 背景オブジェクトの生成禁止範囲
	constexpr float kCreateStopRange = 1500.0f;
	// スカイドームの拡大率
	constexpr float kSkyDomeScale = 2.0f;
}
...
void DemoMap::Update()
{
	...
	for (auto& model : m_stageBackModel)
	{
		if (model->GetPos().z < m_basePos.z - kDeleteRange)
		{
...
void DemoMap::CreateStageBackModel()
{
...
VECTOR DemoMap::GetRandStageBackPos()
{
	// 生成する座標
